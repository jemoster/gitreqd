/**
 * GRD-CLI-009: GitHub release workflow publishes a native Linux x86_64 shallgraph CLI binary.
 */
...
describe("GRD-CLI-009: native CLI binary release artifact", () => {
  it("packaging script builds a Linux x86_64 shallgraph binary into release/", () => {
    const script = fs.readFileSync(PACKAGE_SCRIPT, "utf-8");
    expect(script).toContain("cargo build --release -p shallgraph");
    expect(script).toContain("x86_64-unknown-linux-gnu");
    expect(script).toContain("shallgraph-linux-x86_64");
  ...
  it("release workflow uploads the native binary to GitHub Releases", () => {
    ...
    expect(workflow).toContain("release/shallgraph-linux-x86_64");
  ...
  it("PR cargo-test job uploads the native binary as a downloadable workflow artifact", () => {
    ...
    expect(workflow).toContain("bash ./scripts/package-native-cli.sh");
    expect(workflow).toContain("actions/upload-artifact@v4");
    expect(workflow).toContain("name: shallgraph-linux-x86_64");
    expect(workflow).toContain("path: release/shallgraph-linux-x86_64");
  });
