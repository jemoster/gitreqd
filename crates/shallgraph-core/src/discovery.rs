...
pub const ROOT_MARKER: &str = "shallgraph.yaml";

/// GRD-SYS-007: accepted project root marker filenames (first wins when reading if several exist).
/// Primary names are `shallgraph.yaml` / `shallgraph.yml`; `gitreqd.yaml` / `gitreqd.yml` remain as legacy aliases.
pub const ROOT_MARKER_FILENAMES: [&str; 4] = [
    "shallgraph.yaml",
    "shallgraph.yml",
    "gitreqd.yaml",
    "gitreqd.yml",
...
pub const ROOT_MARKER_HINT: &str = "shallgraph.yaml or shallgraph.yml";

/// Parsed project root marker (`shallgraph.yaml` / `shallgraph.yml`) without filesystem access.
...
/// GRD-SYS-007: prefers `shallgraph.yaml` over `shallgraph.yml` when both exist.
#[cfg(feature = "std-fs")]
...
mod tests {
    ...
    fn temp_dir() -> PathBuf {
        ...
        let dir = std::env::temp_dir().join(format!("shallgraph-disc-{n}-{}", std::process::id()));
    ...
    #[test]
    fn finds_legacy_gitreqd_yaml_marker() {
        let tmp = temp_dir();
        fs::write(tmp.join("gitreqd.yaml"), "requirement_dirs:\n  - reqs\n").unwrap();
        let candidates = discover_project_root_candidates(&tmp).unwrap();
        assert_eq!(candidates.len(), 1);
        assert_eq!(candidates[0], normalize_path(&tmp));
        assert_eq!(
            find_root_marker_path(&tmp).unwrap().file_name().unwrap(),
            "gitreqd.yaml"
        );
    }

    #[test]
    fn prefers_shallgraph_yaml_over_legacy_gitreqd_yaml() {
        let tmp = temp_dir();
        fs::write(tmp.join("gitreqd.yaml"), "requirement_dirs:\n  - legacy\n").unwrap();
