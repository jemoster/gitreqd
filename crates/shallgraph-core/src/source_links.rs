...
fn is_shallgraph_tracing_attr(attr: &Attribute) -> Option<SourceLinkKind> {
    ...
    if crate_name != "shallgraph" && crate_name != "shallgraph_macros" {
        return None;
    }
...
fn emit_links(
    ...
    for attr in attrs {
        let Some(kind) = is_shallgraph_tracing_attr(attr) else {
            continue;
        };
...
"#,
    ...
    fn collects_verifies_on_test_and_multiple_ids() {
        ...
        write_rs(
            ...
            "src/lib.rs",
            r#"
#[shallgraph::verifies("REQ-A", "REQ-B")]
#[test]
...
"#,
    ...
    fn collects_struct_and_nested_module_and_macros_prefix() {
        ...
        write_rs(
            ...
            r#"
mod inner {
    #[shallgraph::implements("REQ-S")]
    pub struct Record {}
}

#[shallgraph_macros::implements("REQ-C")]
