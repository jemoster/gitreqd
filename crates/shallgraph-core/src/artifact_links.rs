...
pub struct GithubArtifactLinkContext {
    ...
    /// Repository-relative posix path to the shallgraph project root (may be empty).
    pub project_root_rel: String,
    /// Absolute shallgraph project root used to strip absolute presented paths. Empty when paths are already project-relative.
...
#[shallgraph::implements("GRD-UI-009")]
...
#[shallgraph::implements("GRD-UI-009", "GRD-HTML-008")]
...
mod tests {
    ...
    fn strips_absolute_paths_before_joining_project_root_rel() {
        let mut ctx = github("acme", "widgets", "abc", "shallgraph");
        ctx.project_root = "/home/dev/src/shallgraph".into();
        assert_eq!(
            github_blob_url_for_artifact(
                "/home/dev/src/shallgraph/requirements/cli/GRD-CLI-001.req.yml",
                &ctx
            ),
            "https://github.com/acme/widgets/blob/abc/shallgraph/requirements/cli/GRD-CLI-001.req.yml"
        );
        assert_eq!(
            github_project_root_rel("/home/dev/src/cloud", "/home/dev/src/cloud/shallgraph"),
            "shallgraph"
        );
        assert_eq!(
